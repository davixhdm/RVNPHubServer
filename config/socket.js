...
const initSocket = (httpServer) => {
  ...
  io.use(async (socket, next) => {
    try {
      ...
      if (user.isSuspended || user.isBanned) return next(new Error('Account suspended or banned'));
      socket.userId = user._id.toString();
      socket.user = {
        id: user._id, firstName: user.firstName, lastName: user.lastName,
        department: user.department, hostel: user.hostel, hdmVerified: user.hdmVerified,
      };
      next();
    } catch (error) {
  ...
  io.on('connection', async (socket) => {
    ...
    if (user.hostel) socket.join(`hostel:${user.hostel}`);
    ...
    socket.on('user:typing', ({ chatId }) => { socket.to(`chat:${chatId}`).emit('user:typing', { chatId, userId, userName: user.firstName }); });
    socket.on('user:stopTyping', ({ chatId }) => { socket.to(`chat:${chatId}`).emit('user:stopTyping', { chatId, userId }); });
