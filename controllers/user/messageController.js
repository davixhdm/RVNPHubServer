...
const getMessages = async (req, res, next) => {
  try {
    const result = await paginate(Message, { chat: req.params.chatId, deletedAt: null }, { page: req.query.page, limit: 30, sort: { createdAt: -1 } });
...
const sendMessage = async (req, res, next) => {
  try {
    ...
    const fileUrl = req.body.fileUrl ? req.body.fileUrl.replace(/&#x2F;/g, '/').replace(/&amp;/g, '&') : null;

    const message = await Message.create({
      chat: req.params.chatId, sender: req.user._id,
      content: req.body.content || '', type: req.body.type || 'text',
      fileUrl, fileName: req.body.fileName || null,
      poll: req.body.poll || null, readBy: [req.user._id],
    });

    chat.lastMessage = { sender: req.user._id, content: req.body.type === 'image' ? '📷 Image' : req.body.type === 'file' ? '📎 File' : (req.body.content?.substring(0, 100) || ''), type: req.body.type || 'text', createdAt: new Date() };

    const unreadCount = { ...chat.unreadCount };
    chat.participants.forEach(p => { const pid = p.toString(); if (pid !== req.user._id.toString()) { unreadCount[pid] = (unreadCount[pid] || 0) + 1; } });
    chat.unreadCount = unreadCount;
    await chat.save();

    socketService.newMessage(req.params.chatId, message);
    chat.participants.forEach(p => { if (p.toString() !== req.user._id.toString()) socketService.emitToUser(p, 'chat:newMessage', message); });
...
const editMessage = async (req, res, next) => {
  try {
    const message = await Message.findOneAndUpdate({ _id: req.params.id, sender: req.user._id }, { content: req.body.content, editedAt: new Date() }, { new: true });
...
const deleteMessage = async (req, res, next) => {
  try {
    const message = await Message.findOneAndUpdate({ _id: req.params.id, sender: req.user._id }, { deletedAt: new Date() }, { new: true });
    if (!message) throw new AppError('Message not found', 404, 'NOT_FOUND');
...
const markAsRead = async (req, res, next) => {
  try {
    ...
    chat.unreadCount = unreadCount;
    await Message.updateMany({ chat: req.params.chatId, sender: { $ne: req.user._id }, readBy: { $ne: req.user._id } }, { $push: { readBy: req.user._id } });
    await chat.save();
...
const createPoll = async (req, res, next) => {
  try {
    const message = await Message.create({ chat: req.params.chatId, sender: req.user._id, type: 'poll', poll: { question: req.body.question, options: req.body.options.map(o => ({ text: o, votes: [] })) } });
    socketService.newMessage(req.params.chatId, message);
...
const votePoll = async (req, res, next) => {
  try {
    ...
    if (!option.votes.includes(req.user._id)) { option.votes.push(req.user._id); await message.save(); }
